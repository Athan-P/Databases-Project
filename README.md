...
When inside the Database, Run via the `setx FLASK_APP "app.py"` command, to set the environment variable `FLASK_APP` to `app.py`, and then use `flask run`. If it doesn't work for `app.py` rename it to `Run.py` and run the above.
